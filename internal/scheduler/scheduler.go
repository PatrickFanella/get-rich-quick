...
func (s *Scheduler) runStrategy(strategy domain.Strategy) {
	s.logger.Info("scheduler: triggered strategy schedule",
		...
		slog.String("ticker", strategy.Ticker),
		slog.String("market_type", strategy.MarketType.String()),
		slog.Time("triggered_at", s.nowFunc().UTC()),
	)

	killSwitchActive, err := s.riskEngine.IsKillSwitchActive(context.Background())
	if err != nil {
	...
	if _, err := s.pipeline.Execute(context.Background(), strategy.ID, strategy.Ticker); err != nil {
